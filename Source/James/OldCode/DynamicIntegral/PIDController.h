...
class PIDController2_DynamicIntegral : public PIDController2
{
	private:
		double m_I_EdgeValue;  //The value to blended on the ends
		double m_I_BlendWidth; //The value used to determine the edge for each side of the set point
	public:
		...
		void SetIEdgeValue(double edgeValue) {m_I_EdgeValue=edgeValue;}
		//If set to zero this feature is bypassed
		void SetIBlendWidth(double width) {m_I_BlendWidth=width;}
		virtual double GetI();
};