...
double PIDController2::operator()(double setpoint,double input,double dTime_s)
{
	if (m_enabled)
	{
		...
		m_result = m_P * m_error + GetI() * m_totalError + m_D * (m_error - m_prevError);
		m_prevError = m_error;
...
void PIDController2::ResetI(double totalError)
{
  ...
  /***********************************************************************************************************/
 /*										PIDController2_DynamicIntegral										*/
/***********************************************************************************************************/


PIDController2_DynamicIntegral::PIDController2_DynamicIntegral(double p, double i, double d,double maximumOutput,double minimumOutput,double maximumInput,
							   double minimumInput,double m_tolerance,bool continuous,bool enabled) :
...
double PIDController2_DynamicIntegral::GetI()
{
	double ret=m_I;
	if (m_I_BlendWidth>0.0)
	{
		//obtain the normalized blend point
		double nbp=fabs(m_totalError);
		//saturate to blend width
		if (nbp>m_I_BlendWidth) 
			nbp=m_I_BlendWidth;
		//Normalize it
		nbp/=m_I_BlendWidth;
		//Now to perform the blend
		//ret= (nbp * m_I_EdgeValue) + ((1.0-nbp)*m_I);
		if (m_error!=0.0)
			printf("e=%.2f te=%.2f ",m_error,m_totalError);
			//printf("e=%.2f blend=%.2f ",m_totalError,ret);
	}
	return ret;
}
