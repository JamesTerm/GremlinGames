...
void FRC_2014_Robot::PitchRamp::BindAdditionalEventControls(bool Bind)
{
  ...
  /***********************************************************************************************************************************/
 /*													FRC_2014_Robot::PowerWheels														*/
/***********************************************************************************************************************************/

FRC_2014_Robot::PowerWheels::PowerWheels(FRC_2014_Robot *pParent,Rotary_Control_Interface *robot_control) : 
...
void FRC_2014_Robot::PowerWheels::BindAdditionalEventControls(bool Bind)
{
	Base::EventMap *em=GetEventMap(); 
	if (Bind)
	{
		em->EventValue_Map["PowerWheels_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC_2014_Robot::PowerWheels::SetRequestedVelocity_FromNormalized);
		em->EventOnOff_Map["PowerWheels_SetEncoderSafety"].Subscribe(ehl,*this, &FRC_2014_Robot::PowerWheels::SetEncoderSafety);
		em->EventOnOff_Map["PowerWheels_IsRunning"].Subscribe(ehl,*this, &FRC_2014_Robot::PowerWheels::SetIsRunning);
	}
	else
	{
		em->EventValue_Map["PowerWheels_SetCurrentVelocity"].Remove(*this, &FRC_2014_Robot::PowerWheels::SetRequestedVelocity_FromNormalized);
		em->EventOnOff_Map["PowerWheels_SetEncoderSafety"].Remove(*this, &FRC_2014_Robot::PowerWheels::SetEncoderSafety);
		em->EventOnOff_Map["PowerWheels_IsRunning"].Remove(*this, &FRC_2014_Robot::PowerWheels::SetIsRunning);
	}
}

void FRC_2014_Robot::PowerWheels::SetRequestedVelocity_FromNormalized(double Velocity) 
{
...
void FRC_2014_Robot::PowerWheels::TimeChange(double dTime_s)
{
	bool IsTargeting=((m_pParent->m_IsTargeting) && GetEncoderUsage()==eActive);
	if (  IsTargeting )
	{
	...
	else
	{
		if ((m_IsRunning)||(m_pParent->m_BallConveyorSystem.GetIsFireRequested()))
		{
			...
			//DOUT5("%f",Velocity);
			size_t DisplayRow=m_pParent->m_RobotProps.GetFRC2014RobotProps().PowerVelocity_DisplayRow;
...
void FRC_2014_Robot::PowerWheels::ResetPos()
{
	m_IsRunning=false;
	__super::ResetPos();
}
  ...
  /***********************************************************************************************************************************/
 /*												FRC_2014_Robot::BallConveyorSystem													*/
/***********************************************************************************************************************************/

FRC_2014_Robot::BallConveyorSystem::BallConveyorSystem(FRC_2014_Robot *pParent,Rotary_Control_Interface *robot_control) : m_pParent(pParent),
...
void FRC_2014_Robot::BallConveyorSystem::Initialize(Base::EventMap& em,const Entity1D_Properties *props)
{
...
void FRC_2014_Robot::BallConveyorSystem::ResetPos() 
{
	m_LowerConveyor.ResetPos(),m_MiddleConveyor.ResetPos(),m_FireConveyor.ResetPos();
	m_ControlSignals.raw=0;
}

void FRC_2014_Robot::BallConveyorSystem::TimeChange(double dTime_s)
{
	...
	if (Fire)
	{
		if (m_FireDelayTriggerOn)
		{
			...
			if (m_FireDelayTrigger_Time>m_pParent->m_RobotProps.GetFRC2014RobotProps().FireTriggerDelay)
	...
	else
	{
		if (m_FireStayOn)
		{
			...
			if (m_FireStayOn_Time>m_pParent->m_RobotProps.GetFRC2014RobotProps().FireButtonStayOn_Time)
				m_FireStayOn=false;
		}
	}
...
void FRC_2014_Robot::BallConveyorSystem::SetRequestedVelocity_FromNormalized(double Velocity)
{
...
void FRC_2014_Robot::BallConveyorSystem::BindAdditionalEventControls(bool Bind)
{
	...
	if (Bind)
	{
		...
		em->EventValue_Map["Ball_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC_2014_Robot::BallConveyorSystem::SetRequestedVelocity_FromNormalized);
		em->EventOnOff_Map["Ball_Fire"].Subscribe(ehl, *this, &FRC_2014_Robot::BallConveyorSystem::Fire);
		em->EventOnOff_Map["Ball_Grip"].Subscribe(ehl, *this, &FRC_2014_Robot::BallConveyorSystem::Grip);
		em->EventOnOff_Map["Ball_GripL"].Subscribe(ehl, *this, &FRC_2014_Robot::BallConveyorSystem::GripL);
		em->EventOnOff_Map["Ball_GripM"].Subscribe(ehl, *this, &FRC_2014_Robot::BallConveyorSystem::GripM);
		em->EventOnOff_Map["Ball_GripH"].Subscribe(ehl, *this, &FRC_2014_Robot::BallConveyorSystem::GripH);
		em->EventOnOff_Map["Ball_Squirt"].Subscribe(ehl, *this, &FRC_2014_Robot::BallConveyorSystem::Squirt);
	}
	else
	{
		em->EventValue_Map["Ball_SetCurrentVelocity"].Remove(*this, &FRC_2014_Robot::BallConveyorSystem::SetRequestedVelocity_FromNormalized);
		em->EventOnOff_Map["Ball_Fire"]  .Remove(*this, &FRC_2014_Robot::BallConveyorSystem::Fire);
		em->EventOnOff_Map["Ball_Grip"]  .Remove(*this, &FRC_2014_Robot::BallConveyorSystem::Grip);
		em->EventOnOff_Map["Ball_GripL"]  .Remove(*this, &FRC_2014_Robot::BallConveyorSystem::GripL);
		em->EventOnOff_Map["Ball_GripM"]  .Remove(*this, &FRC_2014_Robot::BallConveyorSystem::GripM);
		em->EventOnOff_Map["Ball_GripH"]  .Remove(*this, &FRC_2014_Robot::BallConveyorSystem::GripH);
		em->EventOnOff_Map["Ball_Squirt"]  .Remove(*this, &FRC_2014_Robot::BallConveyorSystem::Squirt);
	}
}

  /***********************************************************************************************************************************/
 /*													FRC_2014_Robot::Flippers														*/
/***********************************************************************************************************************************/
FRC_2014_Robot::Flippers::Flippers(FRC_2014_Robot *pParent,Rotary_Control_Interface *robot_control) : 
Rotary_Position_Control("Flippers",robot_control,eFlippers),m_pParent(pParent),m_Advance(false),m_Retract(false)
{
}

void FRC_2014_Robot::Flippers::SetIntendedPosition(double Position)
{
	//DOUT5("Test=%f",RAD_2_DEG(Position));
	__super::SetIntendedPosition(Position);
}

void FRC_2014_Robot::Flippers::TimeChange(double dTime_s)
{
...
void FRC_2014_Robot::Flippers::BindAdditionalEventControls(bool Bind)
{
	Base::EventMap *em=GetEventMap(); //grrr had to explicitly specify which EventMap
	if (Bind)
	{
		em->EventValue_Map["Flippers_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC_2014_Robot::Flippers::SetRequestedVelocity_FromNormalized);
		em->EventValue_Map["Flippers_SetIntendedPosition"].Subscribe(ehl,*this, &FRC_2014_Robot::Flippers::SetIntendedPosition);
		em->EventOnOff_Map["Flippers_SetPotentiometerSafety"].Subscribe(ehl,*this, &FRC_2014_Robot::Flippers::SetPotentiometerSafety);
		em->EventOnOff_Map["Flippers_Advance"].Subscribe(ehl,*this, &FRC_2014_Robot::Flippers::Advance);
		em->EventOnOff_Map["Flippers_Retract"].Subscribe(ehl,*this, &FRC_2014_Robot::Flippers::Retract);
	}
	else
	{
		em->EventValue_Map["Flippers_SetCurrentVelocity"].Remove(*this, &FRC_2014_Robot::Flippers::SetRequestedVelocity_FromNormalized);
		em->EventValue_Map["Flippers_SetIntendedPosition"].Remove(*this, &FRC_2014_Robot::Flippers::SetIntendedPosition);
		em->EventOnOff_Map["Flippers_SetPotentiometerSafety"].Remove(*this, &FRC_2014_Robot::Flippers::SetPotentiometerSafety);
		em->EventOnOff_Map["Flippers_Advance"].Remove(*this, &FRC_2014_Robot::Flippers::Advance);
		em->EventOnOff_Map["Flippers_Retract"].Remove(*this, &FRC_2014_Robot::Flippers::Retract);
	}
}
...
const FRC_2014_Robot::Vec2D c_BridgeDimensions=FRC_2014_Robot::Vec2D(Inches2Meters(48),Inches2Meters(88)); //width x length

const FRC_2014_Robot::Vec2D c_TargetBasePosition=FRC_2014_Robot::Vec2D(0.0,c_HalfCourtLength);
...
const FRC_2014_Robot::Vec2D c_BankShot_Box=FRC_2014_Robot::Vec2D(Inches2Meters(24),Inches2Meters(18)); //width x length
...
void FRC_2014_Robot::Initialize(Entity2D_Kind::EventMap& em, const Entity_Properties *props)
{
	...
	SetDefaultPosition(robot2014props.PresetPositions[m_DefaultPresetIndex]);
}
...
FRC_2014_Robot::BallConveyorSystem &FRC_2014_Robot::GetBallConveyorSystem()
{
	return m_BallConveyorSystem;
}

FRC_2014_Robot::PowerWheels &FRC_2014_Robot::GetPowerWheels()
{
	return m_PowerWheels;
}

void FRC_2014_Robot::ApplyErrorCorrection()
{
	...
	const FRC_2014_Robot_Props::DeliveryCorrectionFields &c00=robot_props.KeyCorrections[0+YOffset][0+XOffset];
	const FRC_2014_Robot_Props::DeliveryCorrectionFields &c01=robot_props.KeyCorrections[0+YOffset][1+XOffset];
	const FRC_2014_Robot_Props::DeliveryCorrectionFields &c10=robot_props.KeyCorrections[1+YOffset][0+XOffset];
	const FRC_2014_Robot_Props::DeliveryCorrectionFields &c11=robot_props.KeyCorrections[1+YOffset][1+XOffset];
...
void FRC_2014_Robot::SetTargetingValue(double Value)
{
...
void FRC_2014_Robot::SetPresetPosition(size_t index,bool IgnoreOrientation)
{
	Vec2D position=m_RobotProps.GetFRC2014RobotProps().PresetPositions[index];
...
void FRC_2014_Robot::Set_Auton_PresetPosition(size_t index)
{
...
void FRC_2014_Robot::SetTarget(Targets target)
{
	m_Target=target;
}

void FRC_2014_Robot::SetDefensiveKeyOn()
{
...
void FRC_2014_Robot::SetPresetPOV (double value)
{
...
void FRC_2014_Robot::Robot_SetCreepMode(bool on) 
{
...
void FRC_2014_Robot::BindAdditionalEventControls(bool Bind)
{
	...
	if (Bind)
	{
		em->EventOnOff_Map["Robot_IsTargeting"].Subscribe(ehl, *this, &FRC_2014_Robot::IsTargeting);
		em->Event_Map["Robot_SetTargetingOn"].Subscribe(ehl, *this, &FRC_2014_Robot::SetTargetingOn);
		em->Event_Map["Robot_SetTargetingOff"].Subscribe(ehl, *this, &FRC_2014_Robot::SetTargetingOff);
		em->EventOnOff_Map["Robot_TurretSetTargetingOff"].Subscribe(ehl,*this, &FRC_2014_Robot::SetTurretTargetingOff);
		em->EventValue_Map["Robot_SetTargetingValue"].Subscribe(ehl,*this, &FRC_2014_Robot::SetTargetingValue);
		em->EventValue_Map["Robot_SetDefensiveKeyValue"].Subscribe(ehl,*this, &FRC_2014_Robot::SetDefensiveKeyPosition);
		em->Event_Map["Robot_SetDefensiveKeyOn"].Subscribe(ehl, *this, &FRC_2014_Robot::SetDefensiveKeyOn);
		em->Event_Map["Robot_SetDefensiveKeyOff"].Subscribe(ehl, *this, &FRC_2014_Robot::SetDefensiveKeyOff);
		em->EventOnOff_Map["Robot_Flippers_Solenoid"].Subscribe(ehl,*this, &FRC_2014_Robot::SetFlipperPneumatic);
		...
		em->Event_Map["Robot_SetPreset1"].Subscribe(ehl, *this, &FRC_2014_Robot::SetPreset1);
		em->Event_Map["Robot_SetPreset2"].Subscribe(ehl, *this, &FRC_2014_Robot::SetPreset2);
		em->Event_Map["Robot_SetPreset3"].Subscribe(ehl, *this, &FRC_2014_Robot::SetPreset3);
		em->EventValue_Map["Robot_SetPresetPOV"].Subscribe(ehl, *this, &FRC_2014_Robot::SetPresetPOV);
		em->EventOnOff_Map["Robot_SetCreepMode"].Subscribe(ehl, *this, &FRC_2014_Robot::Robot_SetCreepMode);
	}
	else
	{
		em->EventOnOff_Map["Robot_IsTargeting"]  .Remove(*this, &FRC_2014_Robot::IsTargeting);
		em->Event_Map["Robot_SetTargetingOn"]  .Remove(*this, &FRC_2014_Robot::SetTargetingOn);
		em->Event_Map["Robot_SetTargetingOff"]  .Remove(*this, &FRC_2014_Robot::SetTargetingOff);
		em->EventOnOff_Map["Robot_TurretSetTargetingOff"].Remove(*this, &FRC_2014_Robot::SetTurretTargetingOff);
		em->EventValue_Map["Robot_SetTargetingValue"].Remove(*this, &FRC_2014_Robot::SetTargetingValue);
		em->EventValue_Map["Robot_SetDefensiveKeyValue"].Remove(*this, &FRC_2014_Robot::SetDefensiveKeyPosition);
		em->Event_Map["Robot_SetDefensiveKeyOn"]  .Remove(*this, &FRC_2014_Robot::SetDefensiveKeyOn);
		em->Event_Map["Robot_SetDefensiveKeyOff"]  .Remove(*this, &FRC_2014_Robot::SetDefensiveKeyOff);
		em->EventOnOff_Map["Robot_Flippers_Solenoid"]  .Remove(*this, &FRC_2014_Robot::SetFlipperPneumatic);
		...
		em->Event_Map["Robot_SetPreset1"]  .Remove(*this, &FRC_2014_Robot::SetPreset1);
		em->Event_Map["Robot_SetPreset2"]  .Remove(*this, &FRC_2014_Robot::SetPreset2);
		em->Event_Map["Robot_SetPreset3"]  .Remove(*this, &FRC_2014_Robot::SetPreset3);
		em->EventValue_Map["Robot_SetPresetPOV"]  .Remove(*this, &FRC_2014_Robot::SetPresetPOV);
		em->EventOnOff_Map["Robot_SetCreepMode"]  .Remove(*this, &FRC_2014_Robot::Robot_SetCreepMode);
	}
...
{
	{
		...
		FRC_2014_Robot_Props::Autonomous_Properties::WaitForBall_Info &ball_1=auton.FirstBall_Wait;
		ball_1.InitialWait=4.0;
		ball_1.TimeOutWait=-1.0;
		ball_1.ToleranceThreshold=0.0;
		FRC_2014_Robot_Props::Autonomous_Properties::WaitForBall_Info &ball_2=auton.SecondBall_Wait;
...
{
	const char *err;
	typedef FRC_2014_Robot_Properties::Vec2D Vec2D;
	double length, width;	
...
const char *ProcessKey(FRC_2014_Robot_Props &m_FRC2014RobotProps,Scripting::Script& script,size_t index)
{
	const char *err;
	typedef FRC_2014_Robot_Properties::Vec2D Vec2D;
	Vec2D PresetPosition;
	err=ProcessVec2D(m_FRC2014RobotProps,script,PresetPosition);
	ASSERT_MSG(!err, err);
	PresetPosition[1]=c_HalfCourtLength-PresetPosition[1];
	m_FRC2014RobotProps.PresetPositions[index]=PresetPosition;  //x,y  where x=width
	return err;
}

const char *ProcessKeyCorrection(FRC_2014_Robot_Props &m_FRC2014RobotProps,Scripting::Script& script,size_t row,size_t column)
{
	const char* err=NULL;
	char CellName[4];
	CellName[0]='c';
	...
	err = script.GetField("p", NULL, NULL,&m_FRC2014RobotProps.KeyCorrections[row][column].PowerCorrection);
	err = script.GetField("x", NULL, NULL,&m_FRC2014RobotProps.KeyCorrections[row][column].YawCorrection);
...
void FRC_2014_Robot_Properties::LoadFromScript(Scripting::Script& script)
{
	...
	if (!err) 
	{
		...
		if (!err) ProcessKey(m_FRC2014RobotProps,script,0);

		err = script.GetFieldTable("key_2");
		if (!err) ProcessKey(m_FRC2014RobotProps,script,1);

		err = script.GetFieldTable("key_3");
		if (!err) ProcessKey(m_FRC2014RobotProps,script,2);

		double fDisplayRow;
		err=script.GetField("ds_display_row", NULL, NULL, &fDisplayRow);
		if (!err)
			m_FRC2014RobotProps.Coordinates_DiplayRow=(size_t)fDisplayRow;
		err=script.GetField("ds_target_vars_row", NULL, NULL, &fDisplayRow);
		if (!err)
			m_FRC2014RobotProps.TargetVars_DisplayRow=(size_t)fDisplayRow;

		err=script.GetField("ds_power_velocity_row", NULL, NULL, &fDisplayRow);
		if (!err)
			m_FRC2014RobotProps.PowerVelocity_DisplayRow=(size_t)fDisplayRow;

		script.GetField("fire_trigger_delay", NULL, NULL, &m_FRC2014RobotProps.FireTriggerDelay);
		script.GetField("fire_stay_on_time", NULL, NULL, &m_FRC2014RobotProps.FireButtonStayOn_Time);
		...
		if (!err)
		{
			for (size_t row=0;row<3;row++)
			{
				for (size_t column=0;column<3;column++)
				{
					err=ProcessKeyCorrection(m_FRC2014RobotProps,script,row,column);
					assert(!err);
				}
			}
			script.Pop();
		...
		if (!err)
		{
			...
			if (!err)
			{
				...
				err=ProcessVec2D(m_FRC2014RobotProps,script,OffsetPosition);
				ASSERT_MSG(!err, err);
			...
			}
			{
				...
				FRC_2014_Robot_Props::Autonomous_Properties::WaitForBall_Info *ballTable[]=
				{
				...
				for (size_t i=0;i<2;i++)
				{
					...
					if (!err)
					{
						FRC_2014_Robot_Props::Autonomous_Properties::WaitForBall_Info &ball=*ballTable[i];
...
FRC_2014_Goals::OperateSolenoid::Goal_Status FRC_2014_Goals::OperateSolenoid::Process(double dTime_s)
{
	...
	{
		case FRC_2014_Robot::eFlipperDown:
			m_Robot.SetFlipperPneumatic(m_IsOpen);
			break;
		case FRC_2014_Robot::eUseLowGear:
		case FRC_2014_Robot::eUseBreakDrive:
		case FRC_2014_Robot::eRampDeployment:
			assert(false);
			break;
	}
	m_Status=eCompleted;
...
{
	const FRC_2014_Robot_Props::Autonomous_Properties &auton=Robot->GetRobotProps().GetFRC2014RobotProps().Autonomous_Props;
	Robot->Set_Auton_PresetPosition(KeyIndex);
	Robot->SetTarget((FRC_2014_Robot::Targets)TargetIndex);
	...
	{
		const Vec2d start_pos=Robot->GetRobotProps().GetFRC2014RobotProps().PresetPositions[KeyIndex];
		WayPoint wp;
	...
	{
		const FRC_2014_Robot_Props::Autonomous_Properties::WaitForBall_Info &ball_1=auton.FirstBall_Wait;
		const FRC_2014_Robot_Props::Autonomous_Properties::WaitForBall_Info &ball_2=auton.SecondBall_Wait;
	...
	{
		DeployFlipper=new OperateSolenoid(*Robot,FRC_2014_Robot::eFlipperDown,true);
...
void FRC_2014_Robot_Control::UpdateVoltage(size_t index,double Voltage)
{
	...
	switch (index)
	{
		...
		case FRC_2014_Robot::eFlippers:
			...
			break;
		case FRC_2014_Robot::ePowerWheels:
...
bool FRC_2014_Robot_Control::GetBoolSensorState(size_t index)
{
	...
	switch (index)
	{
	case FRC_2014_Robot::eLowerConveyor_Sensor:
		ret=m_LowerSensor;
		break;
	case FRC_2014_Robot::eMiddleConveyor_Sensor:
		ret=m_MiddleSensor;
		break;
	case FRC_2014_Robot::eFireConveyor_Sensor:
		ret=m_FireSensor;
		break;
	default:
		assert (false);
...
void FRC_2014_Robot_Control::BindAdditionalEventControls(bool Bind,Base::EventMap *em,IEvent::HandlerList &ehl)
{
	if (Bind)
	{
		em->EventOnOff_Map["Ball_LowerSensor"].Subscribe(ehl, *this, &FRC_2014_Robot_Control::TriggerLower);
		em->EventOnOff_Map["Ball_MiddleSensor"].Subscribe(ehl, *this, &FRC_2014_Robot_Control::TriggerMiddle);
		em->EventOnOff_Map["Ball_FireSensor"].Subscribe(ehl, *this, &FRC_2014_Robot_Control::TriggerFire);
		em->EventOnOff_Map["Ball_SlowWheel"].Subscribe(ehl, *this, &FRC_2014_Robot_Control::SlowWheel);
	}
	else
	{
		em->EventOnOff_Map["Ball_LowerSensor"]  .Remove(*this, &FRC_2014_Robot_Control::TriggerLower);
		em->EventOnOff_Map["Ball_MiddleSensor"]  .Remove(*this, &FRC_2014_Robot_Control::TriggerMiddle);
		em->EventOnOff_Map["Ball_FireSensor"]  .Remove(*this, &FRC_2014_Robot_Control::TriggerFire);
		em->EventOnOff_Map["Ball_SlowWheel"]  .Remove(*this, &FRC_2014_Robot_Control::SlowWheel);
	}
}
...
void FRC_2014_Turret_UI::Initialize(Entity2D::EventMap& em, const Turret_Properties *props)
{
	if (props)
		m_props=*props;
	else
...
void FRC_2014_Turret_UI::UI_Init(Actor_Text *parent)
{
	m_UIParent=parent;
...
void FRC_2014_Turret_UI::update(osg::NodeVisitor *nv, osg::Drawable *draw,const osg::Vec3 &parent_pos,double Heading)
{
	FRC_2014_Control_Interface *turret_access=m_RobotControl;
	double Swivel=(-turret_access->GetRotaryCurrentPorV(FRC_2014_Robot::eTurret));
...
void FRC_2014_Turret_UI::Text_SizeToUse(double SizeToUse)
{
	if (m_Turret.valid()) m_Turret->setCharacterSize(SizeToUse);
}
void FRC_2014_Turret_UI::UpdateScene (osg::Geode *geode, bool AddOrRemove)
{
	if (AddOrRemove)
  ...
  /***************************************************************************************************************/
 /*											FRC_2014_Power_Wheel_UI												*/
/***************************************************************************************************************/

void FRC_2014_Power_Wheel_UI::Initialize(Entity2D::EventMap& em, const Wheel_Properties *props)
{
...
void FRC_2014_Power_Wheel_UI::TimeChange(double dTime_s)
{
	FRC_2014_Control_Interface *pw_access=m_RobotControl;
	double NormalizedVelocity=pw_access->GetRotaryCurrentPorV(FRC_2014_Robot::ePowerWheels) / m_PowerWheelMaxSpeed;
  ...
  /***************************************************************************************************************/
 /*											FRC_2014_Lower_Conveyor_UI											*/
/***************************************************************************************************************/

void FRC_2014_Lower_Conveyor_UI::Initialize(Entity2D::EventMap& em, const Wheel_Properties *props)
{
	Wheel_Properties Myprops;
...
void FRC_2014_Lower_Conveyor_UI::TimeChange(double dTime_s)
{
	FRC_2014_Control_Interface *pw_access=m_RobotControl;
	double Velocity=pw_access->GetRotaryCurrentPorV(FRC_2014_Robot::eLowerConveyor);
	AddRotation(Velocity* 0.5 * dTime_s);
}

  /***************************************************************************************************************/
 /*											FRC_2014_Middle_Conveyor_UI											*/
/***************************************************************************************************************/

void FRC_2014_Middle_Conveyor_UI::Initialize(Entity2D::EventMap& em, const Wheel_Properties *props)
{
	Wheel_Properties Myprops;
...
void FRC_2014_Middle_Conveyor_UI::TimeChange(double dTime_s)
{
	FRC_2014_Control_Interface *pw_access=m_RobotControl;
	double Velocity=pw_access->GetRotaryCurrentPorV(FRC_2014_Robot::eMiddleConveyor);
	AddRotation(Velocity* 0.5 * dTime_s);
}

  /***************************************************************************************************************/
 /*											FRC_2014_Fire_Conveyor_UI											*/
/***************************************************************************************************************/

void FRC_2014_Fire_Conveyor_UI::Initialize(Entity2D::EventMap& em, const Wheel_Properties *props)
{
	Wheel_Properties Myprops;
...
void FRC_2014_Fire_Conveyor_UI::TimeChange(double dTime_s)
{
	FRC_2014_Control_Interface *pw_access=m_RobotControl;
	double Velocity=pw_access->GetRotaryCurrentPorV(FRC_2014_Robot::eFireConveyor);
	AddRotation(Velocity* 0.5 * dTime_s);
}
