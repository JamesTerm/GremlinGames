...
const char * BoardTransLux::Football::GetBoardValue(const char key[])
{
	...
	for (unsigned i=0;i<GetNumFields();i++)
	{
		if (_stricmp(GetFieldName(i),&key[0])==0)
		{
			...
			// 			if ((m_pElement[1].ElementUpdated == true) || (m_pElement[2].ElementUpdated == true))
			// 			{	
			// 				//Sets the individual clock values (min, sec, and tenths) when the clock has changed.
			// 				char * Buffer = (char *)_alloca(8);
			// 
			// 				//Minutes (no hours on TransLux Football)
			// 				strncpy(Buffer, &ret[0], 2);
			// 				Buffer[2] = 0; //zero terminate
			// 				m_pElement[2].ElementName = Buffer;
			// 				m_pElement[2].ElementUpdated = true;
			// 
			// 				//Seconds
			// 				strncpy(Buffer, &ret[3], 2);
			// 				Buffer[2] = 0; //zero terminate
			// 				m_pElement[3].ElementName = Buffer;
			// 				m_pElement[3].ElementUpdated = true;
			// 
			// 				//Tenth of seconds
			// 				strncpy(Buffer, &ret[6], 1);
			// 				Buffer[1] = 0; //zero terminate
			// 				m_pElement[4].ElementName = Buffer;
			// 				m_pElement[4].ElementUpdated = true;
			// 			}
		}
	}
	assert(ret);
