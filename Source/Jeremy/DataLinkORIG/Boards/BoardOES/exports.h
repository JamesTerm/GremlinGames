#ifdef BOARDOES_EXPORTS
#define BOARDOES_API __declspec(dllexport)
#else
#define BOARDOES_API __declspec(dllimport)
#endif
...
// changed.
extern "C" BOARDOES_API HANDLE			live_updates_start( void );
...
extern "C" BOARDOES_API const wchar_t	*live_updates_get_value( const wchar_t key[] );
...
extern "C" BOARDOES_API void			live_updates_release_value( const wchar_t value[] );

// Get the next key that has changed.  When TriCaster™ DataLink™ is done with this wchar_t pointer,
// it will pass it back through live_updates_release_value.
extern "C" BOARDOES_API const wchar_t	*live_updates_get_changed_key( void );

// Same as above, TriCaster™ DataLink™ will release the pointers before it reads
// the next changed key.
extern "C" BOARDOES_API void			live_updates_release_changed_key( const wchar_t key[] );

// All done with stats
extern "C" BOARDOES_API void			live_updates_end( HANDLE hHandle );
