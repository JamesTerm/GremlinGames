...
struct LocalServerData
{
	...
	{
		RakNet::BitStream bstream((unsigned char *) dstream, 50, false);
		int i = 0;
		unsigned char x = 1;
		while(x)
		{
...
int main(void)
{
	...
	printf("RakNet's static data system is used to automatically transfer on-connect data.\n");
	printf("Note: This system is depreciated and will be removed soon\n");
	...
	while (1)
	{
		...
		else if (ch=='p')
		{
			...
			if (rakServer->GetStaticClientData(rakServer->GetPlayerIDFromIndex(0)))
			{
			...
			if (rakClient->GetStaticServerData())
			{
			...
			if ((char *) rakClient->GetStaticClientData(UNASSIGNED_PLAYER_ID))
			{
			...
			pbs = rakClient->GetStaticClientData(rakClient->GetPlayerID());
			if (pbs)
