...
int main(void)
{
	...
	for (i=0; i < CHAIN_LENGTH; i++)
	{
		...
		chain[i]->Initialize(CHAIN_NODES_LENGTH+2, 50000+i, 30,0);
	...
	for (i=0; i < CHAIN_NODES_LENGTH * CHAIN_LENGTH; i++)
	{
		...
		chainNodes[i]->Initialize(1, 60000+i, 30,0);
	...
	while (1)
	{
		...
		if (kbhit())
		{
			...
			if (ch=='s')
			{
				...
				PlayerID recipient;
				char str[256];
				chain[0]->IPToPlayerID("127.0.0.1", 60000, &recipient);
				printf("Sender:\n");
			...
			//	senderChainIndex=0; senderChainNodeIndex=0;
				...
				//	connectionGraph[CHAIN_LENGTH + CHAIN_NODES_LENGTH*senderChainIndex + senderChainNodeIndex].GetGraph()->Print();
					...
					chainNodes[CHAIN_NODES_LENGTH*senderChainIndex + senderChainNodeIndex]->RPC("TestRPC", 0, HIGH_PRIORITY, RELIABLE_ORDERED, 0, systemList.GetList()->operator[](0), false, false, UNASSIGNED_NETWORK_ID, 0);
				}
				else
				{
				//	connectionGraph[senderChainIndex].GetGraph()->Print();
					...
					// TODO
					// Test autorouted RPC
					chain[senderChainIndex]->RPC("TestRPC", 0, HIGH_PRIORITY, RELIABLE_ORDERED, 0, systemList.GetList()->operator[](0), false, false, UNASSIGNED_NETWORK_ID, 0);
				}
				if (outcome)
