...
int main(void)
{
	...
	{
		fullyConnectedMeshPlugin[peerIndex].Initialize(0);
	...
	for (peerIndex=0; peerIndex < NUM_PEERS; peerIndex++)
	{
		rakPeer[peerIndex]->Initialize(NUM_PEERS, 60000+peerIndex, 0);
	}
	...
	for (peerIndex=0; peerIndex < NUM_PEERS; peerIndex++)
	{
		rakPeer[peerIndex]->Disconnect(100);
	}
		...
		if (ch==' ')
		{
			...
			for (i=0; i < NUM_PEERS; i++)
			{
				if (NUM_PEERS<=10)
				{
					...
					{
						playerId=fullyConnectedMeshPlugin[i].GetPeerIDAtIndex(j);
						if (playerId!=UNASSIGNED_PLAYER_ID)
							printf("%i ", playerId.port);
					}
					...
					*/
					...
					{
						playerId=rakPeer[i]->GetPlayerIDFromIndex(j);
						if (playerId!=UNASSIGNED_PLAYER_ID)
							printf("%i ", playerId.port);
					}
