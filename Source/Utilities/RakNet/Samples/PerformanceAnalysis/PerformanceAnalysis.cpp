...
void DoSpeedTest(PacketReliability packetReliability, unsigned sendSize)
{
	RakNetStatisticsStruct *rss1, *rss2;
	...
	peer1->Initialize(1,1234,0,0);
	peer2->Initialize(1,1235,0,0);
	...
	{
		rss1=peer1->GetStatistics(peer1->GetPlayerIDFromIndex(0));
		rss2=peer2->GetStatistics(peer2->GetPlayerIDFromIndex(0));
		if (rss1==0)
		{
		...
		{
			peer1->Send(dummyData,sendSize, HIGH_PRIORITY, packetReliability, 0, UNASSIGNED_PLAYER_ID, true);
			peer2->Send(dummyData,sendSize, HIGH_PRIORITY, packetReliability, 0, UNASSIGNED_PLAYER_ID, true);
		}
		p=peer1->Receive();
		while (p)
		{
	...
	peer1->Disconnect(500);
	peer2->Disconnect(500);
}