...
{
	// I do not like putting this here, but it is a real fight getting through the const jungle
	Surface::db_options_ = db_options_.get();
}
...
void Converter::build_scene_graph(Object &obj)
{
	...
	// We want to make sure that each layer (even empty layers) is represented by 
	// a new group.  To do this, keep track of the last of the layers.
	int lastLayer = -1;
    ...
    for (Object::Layer_map::const_iterator i=obj.layers().begin(); i!=obj.layers().end(); ++i) {
		...
		// Watch for skipped layers, just add an empty group
		while (lastLayer < (layer.number()-1))
		{
			osg::ref_ptr<osg::Group> new_group = new osg::Group;
			lymap[lastLayer] = new_group.get();
			new_group->setName("Empty_layer");
			root_->addChild(new_group.get());
			++lastLayer;
		}
		lastLayer = layer.number();
