...
HBITMAP LoadBMPFromFB( int xsrc, int ysrc, int w, int h );
...
void MovieMaker::StartCapture(const char *name, int x, int y, int w, int h)
{
    strcpy( fname, name );

    // Get the width and height.
    width = w;//glutGet( GLUT_WINDOW_WIDTH  );
    height = h;//glutGet( GLUT_WINDOW_HEIGHT  );
	ulX = x;
	ulY = y;
...
bool MovieMaker::Snap()
{
    ...
    HBITMAP bmp;
    bmp = LoadBMPFromFB( ulX, ulY, width, height );
...
static HBITMAP LoadBMPFromFB( int xsrc, int ysrc, int w, int h )
{
