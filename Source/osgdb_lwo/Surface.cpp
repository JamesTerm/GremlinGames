...
void Surface::generate_stateset(int max_tex_units, bool force_arb_compression) const
{
    if (!stateset_.valid()) {
		...
		if (transparency_ > 0 || (hasTransMap_!=Block::NORMAL)) {
			...
			stateset_->setMode(GL_BLEND,osg:: StateAttribute::ON| osg::StateAttribute::PROTECTED);
        }
        ...
        for (Block_map::const_iterator i=blocks_.begin(); i!=blocks_.end(); ++i) {
            ...
            if (block.get_type() == "IMAP") {
                if (block.get_channel() == "COLR") {
					...
					osg::notify(osg::INFO) << "lwosg::Surface: texture channels of type 'TRAN' are compiled into the COLR map" << std::endl;
				}
				else {
                    osg::notify(osg::WARN) << "lwosg::Surface: texture channels of type '" << block.get_channel() << "' are not supported, this block will be ignored" << std::endl;
                }
            }
        }
    }
