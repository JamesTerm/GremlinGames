...
namespace lwosg
{
	typedef std::map<double, double> Dissolve_Key_map;
    ...
    public:
		...
		struct Dissolve_envelope {
			Dissolve_Key_map keys;
			double singleVal;

			Dissolve_envelope() : singleVal(0.0) {}
		};

    protected:
        bool parse_block(const std::string &name, const std::string &data, const osgDB::ReaderWriter::Options *options);
        bool parse_block(const std::string &name, const std::vector<std::string> &data, const osgDB::ReaderWriter::Options *options);
        void clear();

    private:
        typedef std::map<std::string, osg::ref_ptr<osg::Group> > Object_map;
        static Object_map objects_;
        ...
        struct Scene_object {
            ...
            std::string name;
			Dissolve_envelope ObjectDissolve;
