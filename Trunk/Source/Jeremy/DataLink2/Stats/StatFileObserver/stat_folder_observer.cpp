...
{
#ifndef __VALUE_LENGTH_TESTING__
	size_t MAX_KEY_SIZE = 128;
	...
	case KEY_VALUE_TAG:
		...
		{
#ifndef __VALUE_LENGTH_TESTING__
			// Control the length of the key and value to keep LiveText from choking.
			//TODO: Get Konno to change LiveText to accept and/or deal with larger values. BUG# 33552
			if (m_key_from_xml.length() > MAX_KEY_SIZE) m_key_from_xml.resize(MAX_KEY_SIZE);
			if (m_value_from_xml.length() > MAX_VALUE_SIZE) m_value_from_xml.resize(MAX_VALUE_SIZE);
#endif
		...
		if (*i != NULL_CHAR)
		{
			...
			if (*i == VALUE_CHAR)
			{
				...
				value.assign(begin_value, end_value - begin_value);
 				...
 				// Control the length of the key and value to keep LiveText from choking.
 				//TODO: Get Konno to change LiveText to accept and/or deal with larger values. BUG# 33552
 				if (key.length() > MAX_KEY_SIZE) key.resize(MAX_KEY_SIZE);
 				if (value.length() > MAX_VALUE_SIZE) value.resize(MAX_VALUE_SIZE);
#endif
