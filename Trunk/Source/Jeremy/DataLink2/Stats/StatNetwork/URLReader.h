...
public:
	...
	// Accessors.
	const TICK GetRefreshTime() {return m_refresh;}				// Get the refresh interval.
	const TICK GetTimeLastUpdated() {return m_timeLastUpdated;}	// Get the last time the site was finished being processed.
	const bool GetConnectedState() {return m_isConnected;}		// Get connected flag.
	const wchar_t * GetName() {return m_name;}					// Get the name.
	const std::wstring GetItem() {return m_strFeed;}			// Gets the item string.
	const DWORD GetItemNum() {return m_itemNum;}				// Gets the number of the current item.
