...
{	// FunctionBlock a1(m_PortLock);
	...
	// This is the port name for what is being used
	char l_PortNameToUse[32];
	sprintf(l_PortNameToUse,"\\\\.\\COM%d",Number);
	// Open the serial port.
	m_ComPort=CreateFileA(	l_PortNameToUse,// Pointer to the name of the port
	...
	// Initialize the port settings structure
	m_PortDCB.DCBlength=sizeof(m_PortDCB);
...
{	// The port is not yet open
	...
	m_PortNumber=-1;
	// Setup the port as specified by the user
	SetPort(Number);
}
