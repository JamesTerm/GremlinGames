...
#ifdef STATDATABASE_EXPORTS
#define STATDATABASE_API __declspec(dllexport)
#else
#define STATDATABASE_API __declspec(dllimport)
#endif
...
// changed.
extern "C" STATDATABASE_API HANDLE live_updates_start( void );

// All done with stats.
extern "C" STATDATABASE_API void live_updates_end( HANDLE handle );

// Get the next key that has changed.
extern "C" STATDATABASE_API const wchar_t* live_updates_get_changed_key( void );
...
extern "C" STATDATABASE_API void live_updates_release_changed_key( const wchar_t *key );

// Get a value from a key.
extern "C" STATDATABASE_API const wchar_t* live_updates_get_value( const wchar_t *key );

// Same as above, you need to release the pointer before you can read the next changed value.
extern "C" STATDATABASE_API void live_updates_release_value( const wchar_t *value );
