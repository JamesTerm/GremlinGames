...
bool database::connect_to_databases()
{
	...
	// Make sure we co-initialize.
	if (FAILED(CoInitialize(NULL)))
		return false;
	
	constring = (wchar_t*)(_alloca(sizeof(wchar_t) * (2048)));
	...
	// Create the connections string and get the user / password from registry values.
	...
	// Add query folder to registry path.
	wcscat(path, L"\\Query");

	// Assemble queries from registry values.
	REG_assembleQueries();
...
{
	// Add registry connection to connection list. We can work in parallel to the file based database definitions.
	bool ret = true;
