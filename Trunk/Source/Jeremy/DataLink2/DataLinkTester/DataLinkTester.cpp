...
// 	va_end(marker); 
	...
	vsprintf_s(Temp,8192,format,marker);
...
{
	#ifdef _DEBUG
	printf(	"*All output directed to VS debug Output Window*\n");
	#endif
	printf(	"test \t\t\tShow scoreboard info.\n"
			...
			"\t\t\t(0=Aggregator 1=DLClock 2=Database 3=FileObserver\n"
			"\t\t\t 4=Network\n"
...
bool GetFunctionPointers()
{
	...
	catch (int ErrorCode)
	{
		...
		#else
		printf("GetFunctionPointers failed in Aggregator: %d\n", ErrorCode);
		#endif
	}
	return Success;
}
