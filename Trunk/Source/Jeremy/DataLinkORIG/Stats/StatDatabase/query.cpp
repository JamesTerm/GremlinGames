	...
	if (m_connection->open() != NULL)
	{
		try
		{
			...
			if (!recordset->ADOEOF)
			{
				...
				if (variant.vt!=VT_NULL)	// Avoid SQL statements which are unsupported / return unsupported results.
				{
					if (variant.bstrVal)
					{
...
// 					assert false;
	...
	CoUninitialize();	// CoUnInitialize added because of com error from the recordset.CreateInstance
