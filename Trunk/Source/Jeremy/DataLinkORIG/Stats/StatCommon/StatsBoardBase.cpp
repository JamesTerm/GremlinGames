...
void StatsBoardBase::ReadFromScoreBoard(void)
{
	...
	lNumBytesRead = m_pSerialPort->ReadFromPort( m_SerialInBuffer, 1);				// NEW
//	lNumBytesRead = m_pSerialPort->ReadFromPort( m_SerialInBuffer, m_PacketSize);	// OLD
...
// #ifdef _DEBUG
	if (m_UseTextFile)
		Sleep(20);	//Testing...add delay for text input test
// #endif
}
