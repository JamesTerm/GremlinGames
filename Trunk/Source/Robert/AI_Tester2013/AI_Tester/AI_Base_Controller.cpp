...
namespace
{
	Vec2d DirectionFromHeading(double _heading)
	{
		// assuming radians.
		Vec2d dir;

		dir._v[0] = -sin(_heading);
		dir._v[1] = cos(_heading);

		return dir;
	}

	double Hermite1d(double x0, double x1, double dx0, double dx1, double t)
	{
	...
	Vec2d Hermite(Vec2D const & _x0, Vec2d const & _d0, Vec2D const & _x1, Vec2d const & _d1, double _t)
	{
		return Vec2d(Hermite1d(_x0.x(), _x1.x(), _d0.x(), _d1.x(), _t), Hermite1d(_x0.y(), _x1.y(), _d0.y(), _d1.y(), _t));
	}
}
  /***********************************************************************************************************************************/
...
Goal::Goal_Status Goal_Ship_FollowShip::Process(double dTime_s)
{
	...
	if (m_Status==eActive)
	{
		...
		if (true)
		{
			...
			Vec2d followHeading = DirectionFromHeading(m_Followship.GetPhysics().GetHeading());
			Vec2d shipHeading = DirectionFromHeading(m_ship.GetPhysics().GetHeading());
			double dotDirection = followHeading * shipHeading;
			/*
			Vec2d turnPoint = m_Followship.GetPos_m() - (followHeading * (m_ship.GetDimensions().length() + m_Followship.GetDimensions().length()));

			globalGoalPositionPoint = globalGoalTrajectoryPoint = Hermite(m_ship.GetPos_m(), shipHeading, turnPoint, followHeading, 0.5);
			*/
			globalGoalPositionPoint = globalGoalTrajectoryPoint = m_Followship.GetPos_m();
			Vec2d globalGoalVelocity = m_Followship.GetPhysics().GetLinearVelocity();
			...
			double minvel = 0.5;
			if (m_Followship.GetPhysics().GetLinearVelocity().length() < minvel && m_ship.GetPhysics().GetLinearVelocity().length() < minvel)
				m_ship.SetIntendedOrientation(m_Followship.GetPhysics().GetHeading());
		}
		else
		{
