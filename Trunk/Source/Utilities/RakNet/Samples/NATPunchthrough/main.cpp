...
void main(void)
{
	...
	{
		rakPeer->Initialize(2,60063,0,0);
	...
	{
		rakPeer->Initialize(2,60064,0,0);
	...
	{
		rakPeer->Initialize(2,60065,0,0);
	...
	while (1)
	{
		...
		if (kbhit())
		{
			...
			if (ch=='c' && (mode[0]=='s' || mode[0]=='S'))
			{
				...
				// assume is the first system connected (GetPlayerIDFromIndex(0))
				natPunchthrough.Connect(recipientIP, 60064, 0, 0, rakPeer->GetPlayerIDFromIndex(0));
