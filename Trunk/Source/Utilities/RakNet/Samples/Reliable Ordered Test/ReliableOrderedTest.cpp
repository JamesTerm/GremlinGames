	...
	while (currentTime < quitTime)
	{
		if (kbhit())
		{
			...
			else if (ch==' ')
			{
				...
				char message[2048];
				if (sender)
					rss=sender->GetStatistics(sender->GetPlayerIDFromIndex(0));
				else
					rss=receiver->GetStatistics(receiver->GetPlayerIDFromIndex(0));
		...
		if (sender)
		{
			...
			//	streamNumber = randomMT() % 32;
				...
				char *pad;
				int padLength = (randomMT() % 1000) + 1;
				pad = new char [padLength];
				bitStream.Write(pad, padLength);
				delete [] pad;
				// Send on a random priority with a random stream
				if (sender->Send(&bitStream, HIGH_PRIORITY, (PacketReliability) (RELIABLE + (randomMT() %2)) ,streamNumber, UNASSIGNED_PLAYER_ID, true)==false)
