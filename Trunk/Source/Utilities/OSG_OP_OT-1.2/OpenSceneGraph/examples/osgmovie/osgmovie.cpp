...
int main(int argc, char** argv)
{
	...
	std::string folder("C:\\Projects NG\\LEP Animation Project\\FullMovie\\Renders\\CockpitGlare_51_375\\");
	std::string fileBase("glare");
	...
	{
		if (i < 10)
			sprintf(imageNum, "000%i", i);
		else if (i < 100)
			sprintf(imageNum, "00%i", i);
		else if (i < 1000)
			sprintf(imageNum, "0%i", i);

		std::string fn = folder+fileBase+imageNum+fileExt;
		...
		{
			std::cout<<"Unable to read file "<<fn<<std::endl;
		}            
	}
