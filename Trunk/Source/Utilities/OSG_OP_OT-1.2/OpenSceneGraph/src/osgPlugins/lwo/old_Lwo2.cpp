...
{
    notify(NOTICE)  << "Opening file: " << filename << std::endl;

    _fin.open(filename.c_str(), ios::in | ios::binary );
    if (!_fin.is_open())
    {
        notify(FATAL) << "Can't open file '" << filename << "'" << std::endl;
        return false;
    }
    ...
    {
        notify(FATAL) << "File '" << filename << "' is not IFF format file." << std::endl;
        _fin.close();
        return false;
    }
    else 
    ...
    if (_read_long() != tag_LWO2) 
    {
        ...
        notify(FATAL) << "File '" << filename << "' is not LWO2 format file." << std::endl;
        _fin.close();
        return false;
    }
    else 
...
Lwo2::_generate_statesets_from_surfaces()
{
    ...
    for (IteratorSurfaces itr_surf = _surfaces.begin(); itr_surf != _surfaces.end(); itr_surf++)
    {
        ...
        {
			osg::notify(osg::NOTICE) << "Reading Image: " << _images[surface->image_index] << std::endl;
            Image* image = osgDB::readImageFile(_images[surface->image_index]);
			if (!image)
				osg::notify(osg::FATAL) << "Failed To Read Image: " << _images[surface->image_index] << std::endl;
