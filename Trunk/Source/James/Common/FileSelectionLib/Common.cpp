...
size_t LoadDefaults(const char *DefaultFileName)
{
	...
	string GroupID;
	size_t Selection;
	size_t DefaultConsole; //hmmm I have to use local variables for IOstream
	bool UsedManualFileName;
	//wow this is neat
	in>>GroupID>>Selection>>DefaultConsole>>UsedManualFileName;
	//find the group
...
void SaveDefaults(const char *DefaultFileName,size_t DefaultConsole)
{
	//Save selections
	...
	out << endl;
	out << (unsigned int)g_FileStuff->DefaultSelection << endl;
	out << (unsigned int)DefaultConsole << endl;
