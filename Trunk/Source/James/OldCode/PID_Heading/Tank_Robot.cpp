...
Tank_Robot::Tank_Robot(const char EntityName[],Tank_Drive_Control_Interface *robot_control,bool IsAutonomous) :
	...
	m_PIDController_Heading(0.0,0.0,0.0),
	#ifdef __Tank_UseScalerPID__
	m_UsingEncoders(IsAutonomous),m_IsAutonomous(IsAutonomous),
	m_VoltageOverride(false),
	#else
	m_ErrorOffset_Left(0.0),m_ErrorOffset_Right(0.0),m_ErrorOffset_Heading(0.0),
...
void Tank_Robot::Initialize(Entity2D::EventMap& em, const Entity_Properties *props)
{
	...
	//These are not bound to voltage
	m_PIDController_Heading.SetInputRange(-dHeading,dHeading);
	m_PIDController_Heading.SetOutputRange(-dHeading,dHeading);
	m_PIDController_Heading.Enable();
	#ifdef __Tank_UseScalerPID__
	...
	m_PID_Input_Latency_Heading.SetLatency(m_TankRobotProps.InputLatency);
}
...
void Tank_Robot::InterpolateThrusterChanges(Vec2D &LocalForce,double &Torque,double dTime_s)
{
	...
	//Compute error for the heading
	{
		...
		#endif
		//TODO see if we need to add tolerance
	}
