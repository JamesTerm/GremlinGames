...
{
	public:
		///Currently the AudioOutput has no effect as currently have no plans for multiple channels; however, this is left in place
		///should those plans change.  The UseDisplayedCallbacks should be true if this is going to drive the clock from the audio mixer
		///If for example you wish to run this along side the hardware it should be false (this is good for latency testing)
		DS_Output(size_t AudioOutput=0,
			const wchar_t AudioName[]=L"DS_Output_Audio",
			bool UseDisplayedCallbacks=true  ///< This should always be true unless this is a "thru" output (i.e. SDI)
			);
		~DS_Output();
