...
namespace osgUtil
{
	...
	{
		public:
			void setRenderStageLighting(osg::ref_ptr<PositionalStateContainer> lighting)
			{
				_renderStageLighting=lighting;
			}
			osg::ref_ptr<PositionalStateContainer> getRenderStageLighting()
			{
				return _renderStageLighting;
			}
			RenderStageList getRenderGraphList()
			{
				return _postRenderList;
			}
	};
}
...
{   
    ...
    // will do later.
    osgUtil::RenderToTextureStage* previous_stage = dynamic_cast<osgUtil::RenderToTextureStage*>(cv.getCurrentRenderBin()->getStage());
