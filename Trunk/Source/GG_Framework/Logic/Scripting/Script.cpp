...
const char* Script::GetGlobal(std::string globalName, 
	std::string* t_string, bool* t_boolean, double* t_number)
{
	...
	if (err)
	{
		...
		return CacheError(GG_Framework::Base::BuildString("LUA Script[%s] failed to read %s, {%s}\n", 
...
const char* Script::GetStackValue(
	std::string* t_string, bool* t_boolean, double* t_number)
{
...
const char* Script::GetGlobalTable(std::string globalTableName)
{
	...
	{
		return CacheError(GG_Framework::Base::BuildString(
...
//////////////////////////////////////////////////////////////////////////
const char* Script::GetField(std::string fieldName, 
	std::string* t_string, bool* t_boolean, double* t_number)
{
	ASSERT(IsValid());
	ASSERT(lua_istable(m_lua_state, -1));	// We should already have a stack
	...
	{
		std::string errMsg(err);
		return CacheError(GG_Framework::Base::BuildString("LUA Script[%s] failed to read field name %s, {%s}\n", 
		...
		std::string* t_string, bool* t_boolean, double* t_number)
{
	...
	{
		std::string errMsg(err);
		return CacheError(GG_Framework::Base::BuildString("LUA Script[%s] failed to read field index %i, {%s}\n", 
...
const char* Script::CallFunction(const char *func, const char *sig, ...) 
{
	...
	while (*sig) {  /* push arguments */
		switch (*sig++) {
		  ...
		  default:
			  va_end(vl);
			  return CacheError(GG_Framework::Base::BuildString("invalid option (%c)\n", *(sig - 1)), narg);
		}
		narg++;
	...
	{
		va_end(vl);
		return CacheError(GG_Framework::Base::BuildString("error running function `%s': %s\n",
	...
	while (*sig) {  /* get results */
		switch (*sig++) {
		  ...
		  case 'd':  /* double result */
			  ...
			  {
				  va_end(vl);
				  return CacheError(GG_Framework::Base::BuildString("Invalid return type for %s at pos %i, expected number\n",
					  func, -resIndex), nres);
			  }
		  ...
		  case 'i':  /* int result */
			  ...
			  {
				  va_end(vl);
				  return CacheError(GG_Framework::Base::BuildString("Invalid return type for %s at pos %i, expected int\n",
					  func, -resIndex), nres);
			  }
		  ...
		  case 'b':  /* bool result */
			  ...
			  {
				  va_end(vl);
				  return CacheError(GG_Framework::Base::BuildString("Invalid return type for %s at pos %i, expected bool(int)\n",
					  func, -resIndex), nres);
			  }
		  ...
		  case 's':  /* string result */
			  ...
			  {
				  va_end(vl);
				  return CacheError(GG_Framework::Base::BuildString("Invalid return type for %s at pos %i, expected string\n",
					  func, -resIndex), nres);
			  }
		  ...
		  case 't':	/* Table result */
			  ...
			  {
				  va_end(vl);
				  return CacheError(GG_Framework::Base::BuildString("Invalid return type for %s at pos %i, expected table\n",
					  func, -resIndex), nres);
			  }
